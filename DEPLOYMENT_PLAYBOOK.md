...
|---|---|---|---|
| 1 | `FATAL ERROR: Reached heap limit — JavaScript heap out of memory` during install/build | Heavy install/build in the memory-constrained sandbox; full production builds of a large app (Monaco, workers) can exceed the limit | Use **Bun** for everything (`bun install`, `bun run`). Never run `vite build` in the sandbox — `bun run verify` (codegen + `tsc -b --noEmit` + tests) is the gate. Routes stay lazy-loaded so the bundle stays split. Add large dependencies only when necessary |
